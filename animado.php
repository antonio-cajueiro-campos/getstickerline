...
<body>
    <div class="container-fluid">
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <a href="animado.php" class="navbar-brand"><span class="navbar-brand mb-0 h1"><span class="title">Line Stickers Downloader</span> // Animated Mode</span></a>
            ...
            <div class="collapse navbar-collapse" id="navbar-site">
                <ul class="navbar-nav ml-auto">
                    <a target="_blank" href="https://store.line.me/stickershop/showcase/top_creators/pt-BR?category=2000" class="btn nav-link text-light btn-dark ols">Sticker Animados</a>
                    <a href="index.php" class="btn nav-link text-light btn-dark">Versão para Sticker estáticos</a>
                </ul>
            </div>
        </nav>
        <div class="container">
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="" method="POST">
                            ...
                            <input type="text" name="ursite" id="se" class="form-control" placeholder="Ex: https://store.line.me/stickershop/product/12856323">
                        ...
                        <?php
                            ...
                            if (isset($_POST['postar'])) {
                                ...
                                if (!empty($site))
                                    if (copy($site, 'tempsite.txt')) {
                                        ...
                                        if (preg_match('/https:\/\/stickershop\.line-scdn\.net\/stickershop\/v1\/sticker\/[0-9]+\/iPhone\/sticker_animation@2x\.png/', $pagina, $combina) > 0 ) {
                                            $imagem = $combina[0];
                                            $imagem = str_replace("https://stickershop.line-scdn.net/stickershop/v1/sticker/", "", $imagem);
                                            $imagem = str_replace("/iPhone/sticker_animation@2x.png", "", $imagem);
                                            unlink('tempsite.txt');
                                            $qt = preg_match_all('/FnStickerPreviewItem/', $pagina, $useless);
                                            header("location: animado.php?img=$imagem&qt=$qt&id_page=$id_pages");
                                        } else {
                                            unlink('tempsite.txt');
                                            echo "<div style=margin-top:15px;>Stickers não encontrados, veja se digitou um <span style=text-decoration:underline;cursor:pointer; title='Exemplo: https://store.line.me/stickershop/product/12856323' onclick='tip(1)'>link válido...</span></div>";
                                        }
                                    } else
                            ...
                            if (isset($_GET['img'])) {
                                for ($i = 0; $i < $qt; $i++) {
                                    $url = "https://stickershop.line-scdn.net/stickershop/v1/sticker/".$id."/iPhone/sticker_animation@2x.png";
                                    $p = $i % 4;
                                    if ($p == 0 && $i == 0)
